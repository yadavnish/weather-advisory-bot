...
async def resolve_location(state: GraphState) -> GraphState:
    ...
    try:
        ...
        return {**state, "latitude": lat, "longitude": lon, "location_name": resolved_name}
...
async def fetch_weather_node(state: GraphState) -> GraphState:
    if state.get("error"):
        return state
    try:
        snapshot = await fetch_weather(state["latitude"], state["longitude"], state["location_name"])
...
async def check_systemic_override(state: GraphState) -> GraphState:
    if state.get("error"):
        return state
    override_sops = [s for s in state["sops"] if s.match_type == "systemic_override"]
    match = None
    for sop in override_sops:
        m = sop_engine.match_systemic_override(sop, state["snapshot"])
        if m.matched:
            match = m
            break
...
async def match_threshold_sops(state: GraphState) -> GraphState:
    ...
    threshold_sops = [s for s in state["sops"] if s.match_type == "threshold"]
    matches = [
        sop_engine.match_threshold_sop(sop, state["question"], state["snapshot"], now_hour)
...
async def match_semantic_sops(state: GraphState) -> GraphState:
    if state.get("error"):
        return state
    semantic_sops = [s for s in state["sops"] if s.match_type == "semantic"]
    matches: list[SOPMatch] = []
    for sop in semantic_sops:
        result = await classify(state["question"], sop, state["snapshot"])
        matches.append(
            SOPMatch(
                sop=sop,
                matched=result.applies and result.confidence >= 0.5,
                rationale=result.raw_error or f"llm classifier confidence={result.confidence}",
...
async def rank_and_select_node(state: GraphState) -> GraphState:
    if state.get("error"):
        return state
    all_matches = list(state.get("threshold_matches", [])) + list(state.get("semantic_matches", []))
    override = state.get("override_match")
    if override is not None:
        all_matches.append(override)

    primary, others = sop_engine.rank_and_select(all_matches)
    return {**state, "all_matches": all_matches, "primary": primary, "others": others}
...
async def compose_answer(state: GraphState) -> GraphState:
    ...
    if primary is None:
        return {**state, "error": "no_policy_match"}

    primary_text = sop_engine.render_template(primary.sop, snapshot, primary.llm_summary)
    ...
    if override is not None and override.matched and primary.sop.id != override.sop.id:
        override_text = sop_engine.render_template(override.sop, snapshot)
        primary_text = f"{override_text}\n\n{primary_text}"

    others = [m for m in state.get("others", []) if m.sop.id != primary.sop.id]
    if others:
        disclosures = "; ".join(f"{m.sop.id} (severity {m.sop.severity})" for m in others)
        primary_text += f"\n\n[Also applicable, not primary: {disclosures}]"

    return {**state, "answer_text": primary_text}
...
async def validate_grounding(state: GraphState) -> GraphState:
    ...
    if state.get("override_match"):
        matched_sops.append(state["override_match"].sop)
    matched_sops.extend(m.sop for m in state.get("others", []))
    for sop in matched_sops:
        allowed |= sop_engine.collect_condition_value_tokens(sop)

    # Severity numbers and SOP-id numbers are meta, not weather facts -- skip.
    skip_context = re.compile(r"(severity|SOP-)\s*\d*")
    scrubbed = skip_context.sub("", answer)

    found_numbers = set(_NUMBER_RE.findall(scrubbed))
    ungrounded = {n for n in found_numbers if n not in allowed}

    if ungrounded:
        return {
            **state,
            "error": f"grounding_failed: numbers {ungrounded} not in weather snapshot",
        }
    return state
...
def _terminal_response(state: GraphState) -> GraphState:
    ...
    if error is None:
        ...
        session.last_sop_ids = [primary.sop.id] + [m.sop.id for m in others]

        response = BotResponse(
            answer=state["answer_text"],
            primary_sop_id=primary.sop.id,
            other_matched_sop_ids=[m.sop.id for m in others],
    ...
    if error == "no_location":
        msg = "I need a location to check conditions - which city or place?"
    elif error.startswith("location_error"):
        msg = f"I couldn't resolve that location, so I can't give you a verified answer. ({error})"
    elif error.startswith("weather_error"):
        msg = f"I couldn't fetch live weather data, so I can't give you a verified answer. ({error})"
    elif error == "no_policy_match":
        msg = "We don't have specific guidance covering that scenario yet, so I won't guess."
    elif error.startswith("grounding_failed"):
        msg = "I couldn't produce a fully verified answer for that (a value didn't trace back to the weather data), so I'm holding back rather than risk a wrong number."
    else:
        msg = "Something went wrong and I can't give you a verified answer right now."

    response = BotResponse(
        answer=msg,
        primary_sop_id=None,
        other_matched_sop_ids=[],
        weather_snapshot=state.get("snapshot").as_dict() if state.get("snapshot") else None,
        grounded=False,
...
def _route_after_location(state: GraphState) -> str:
    return "honest_fallback" if state.get("error") else "fetch_weather"


def _route_after_weather(state: GraphState) -> str:
    return "honest_fallback" if state.get("error") else "check_systemic_override"
...
def _route_after_validate(state: GraphState) -> str:
    return "honest_fallback" if state.get("error") else "finalize"
...
def build_graph():
    ...
    g.add_node("check_systemic_override", check_systemic_override)
    g.add_node("match_threshold_sops", match_threshold_sops)
    g.add_node("match_semantic_sops", match_semantic_sops)
    g.add_node("rank_and_select", rank_and_select_node)
    g.add_node("compose_answer", compose_answer)
    g.add_node("validate_grounding", validate_grounding)
    g.add_node("honest_fallback", _terminal_response)
    g.add_node("no_policy_fallback", _terminal_response)
    g.add_node("finalize", _terminal_response)

    g.set_entry_point("resolve_location")

    g.add_conditional_edges(
        "resolve_location", _route_after_location, {"honest_fallback": "honest_fallback", "fetch_weather": "fetch_weather"}
    )
    g.add_conditional_edges(
        "fetch_weather",
        _route_after_weather,
        {"honest_fallback": "honest_fallback", "check_systemic_override": "check_systemic_override"},
    )
    g.add_edge("check_systemic_override", "match_threshold_sops")
    g.add_edge("match_threshold_sops", "match_semantic_sops")
    g.add_edge("match_semantic_sops", "rank_and_select")
    g.add_conditional_edges(
        "rank_and_select",
        _route_after_rank,
        {"no_policy_fallback": "no_policy_fallback", "compose_answer": "compose_answer"},
    )
    g.add_edge("compose_answer", "validate_grounding")
    g.add_conditional_edges(
        "validate_grounding", _route_after_validate, {"honest_fallback": "honest_fallback", "finalize": "finalize"}
    )

    g.add_edge("honest_fallback", END)
    g.add_edge("no_policy_fallback", END)
